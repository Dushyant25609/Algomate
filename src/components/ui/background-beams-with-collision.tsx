...
export const BackgroundBeamsWithCollision = ({
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        'fixed inset-0 -z-50 flex items-center w-full justify-center overflow-hidden',
        className
      )}
    >
      {beams.map(beam => (
...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  const spans = Array.from({ length: 50 }, (_, index) => ({
    ...
    initialX: 0,
    initialY: 0,
    directionX: Math.floor(Math.random() * 100 - 40),
    directionY: Math.floor(Math.random() * -250 - 10),
  }));
