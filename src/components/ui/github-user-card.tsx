...
const GithubUserCard: FC<GithubUserCardProps> = ({ github }) => {
  ...
  const totalStars = github.repos
    ? github.repos.reduce((sum, repo) => sum + repo.stargazers_count, 0)
    : 0;
  const totalForks = github.repos
    ? github.repos.reduce((sum, repo) => sum + repo.forks_count, 0)
    : 0;
  return (
    <motion.div
    ...
    >
              ...
              {github.avatar_url && (
                <div className="relative">
                  ...
                  <div className="absolute -bottom-1 -right-1 z-20 bg-background rounded-full p-1 shadow-sm border border-border">
                    <Github className="h-2 w-2 text-primary" />
                  </div>
                </div>
              )}
            </div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3 text-xs md:text-sm">
        <div className="relative overflow-hidden group rounded-lg">
          ...
          <div className="flex items-center gap-2 p-2 backdrop-blur-sm relative z-10 h-full  rounded-lg shadow-sm group-hover:shadow-md transition-all duration-300">
